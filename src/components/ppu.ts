...
export class PPU {
    ...
    nmiCounter: number
    scanlineAlreadyRendered: boolean
    f_nmiOnVblank: number
    f_spriteSize: number
    f_bgPatternTable: number
    f_spPatternTable: number
    f_addrInc: number
    f_nTblAddress: number
    f_color: number
    f_spVisibility: number
    f_bgVisibility: number
    f_spClipping: number
    f_bgClipping: number
    f_dispType: number
    cntFV: number
    ...
    private palTable: PaletteTable

    // Rendering Options:
    showSpr0Hit: boolean
    clipToTvSize: boolean

    constructor(private cpu: CPU, private writeFrame: (buffer: number[]) => void) {
        ...
        for (let i = 0; i < this.vramMem.length; i += 1) {
            this.vramMem[i] = 0;
        }
        for (let i = 0; i < this.spriteMem.length; i += 1) {
            this.spriteMem[i] = 0;
        }
    ...
    public setMirroring(mirroring: number ) {
    ...
    private defineMirrorRegion(fromStart: number, toStart: number, size: number) {
    ...
    setStatusFlag(flag: StatusFlag, value: boolean) {
        var n = 1<<flag;
        this.cpu.mem[0x2002] =
...
export class Tile {
    ...
    setBuffer(scanline: number[]): void {
    ...
    render(buffer: number[], srcx1: number, srcy1: number, srcx2: number, srcy2: number, dx: number, dy: number, palAdd: number, palette: number[], flipHorizontal: boolean, flipVertical: boolean, pri: number, priTable: number[]): void {
